...
namespace ktl
{
	...
	public:
		...
		V& at(const K& index) const
		{
			...
			KTL_ASSERT(block != m_End && detail::flag_occupied_alive(block->Flags));
	...
	private:
		...
		pair* get_pair(const K& index, pair* begin, size_t mask) const noexcept
		{
			...
			} while (counter < cap && detail::flag_occupied(block->Flags) && (detail::flag_dead(block->Flags) || !Equals()(block->Key, index)));
